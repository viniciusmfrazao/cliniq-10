...
async function scheduleNextFollowup(payload: IncomingPayload, ctx: DonnaContext, opts: {
  ...
  if (opts.isFollowupRun) {
    ...
    if (newCount >= 6) {
      // Esgotou tentativas (mandou 5 follow-ups: 2h, 4h, 48h, 5d, 10d)
      await fetchJson(`${SUPABASE_URL}/rest/v1/leads?id=eq.${ctx.lead.id}`, {
        method: 'PATCH',
        headers: {
          apikey: SERVICE_ROLE_KEY,
...
Deno.serve(async (req) => {
  ...
  // ─── Interceptação de resposta de botão (CONFIRMAR / CANCELAR / NÃO SOU EU) ───
  //
  // Quando a clínica envia lembrete D-1 ou 2h com botões interativos,
  // a resposta do paciente chega como texto (ex: "✅ Confirmar").
  // Processamos direto SEM chamar Claude: mais rápido, sem custo de IA.
  if (!payload.isFollowup) {
    const btnNorm = (payload.userText || '').trim()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '') // remove acentos
      .replace(/[^\p{L}\s]/gu, '')                       // remove emoji/pontuação
      .trim().toUpperCase();
    ...
    if (isConfirmar || isCancelar || isNaoSouEu) {
      const patientFirst = ((ctx.patient?.name || ctx.lead?.name || '')).trim().split(/\s+/)[0] || '';

      if (isConfirmar && ctx.patient?.id) {
        try {
          const aptUrl = `${SUPABASE_URL}/rest/v1/appointments?patient_id=eq.${ctx.patient.id}&clinic_id=eq.${payload.clinicId}&status=in.(scheduled,pending_confirmation)&start_time=gte.${new Date().toISOString()}&order=start_time.asc&limit=1&select=id,start_time`;
          const aptR = await fetchJson<Array<{ id: string; start_time: string }>>(aptUrl, {
            method: 'GET',
            headers: { apikey: SERVICE_ROLE_KEY, Authorization: `Bearer ${SERVICE_ROLE_KEY}` },
          });
          if (aptR.ok && Array.isArray(aptR.data) && aptR.data.length > 0) {
            const apt = aptR.data[0];
            await fetchJson(`${SUPABASE_URL}/rest/v1/appointments?id=eq.${apt.id}`, {
              method: 'PATCH',
              headers: {
                apikey: SERVICE_ROLE_KEY,
                Authorization: `Bearer ${SERVICE_ROLE_KEY}`,
            ...
            const msg = patientFirst
              ? `${patientFirst}, presença confirmada! ✅\n\nTe esperamos ${dateStr} às ${timeStr}. Vai ser ótimo te receber! 💜`
              : `Presença confirmada! ✅\n\nTe esperamos ${dateStr} às ${timeStr}. Vai ser ótimo te receber! 💜`;
            await sendViaEvolution(payload, ctx, msg);
            console.log(JSON.stringify({ evt: 'button_confirmar', apt: apt.id, clinic: payload.clinicId }));
            return jsonResponse({ ok: true, action: 'button_confirmar', appointment_id: apt.id });
          }
        } catch (e) {
          errors.push(`button_confirmar: ${(e as Error).message}`);
          // fallthrough pro Claude se der erro
        }
      }

      if (isCancelar) {
        try {
          await executeToolByName('escalar_humano', {
            motivo: 'cancelamento',
            detalhes: 'Paciente clicou em CANCELAR no lembrete de confirmação de agendamento.',
          }, ctx, payload, { supabaseUrl: SUPABASE_URL, serviceKey: SERVICE_ROLE_KEY });
          const msg = patientFirst
            ? `${patientFirst}, entendi! Vou avisar nossa equipe para entrar em contato. 💜`
            : `Entendi! Vou avisar nossa equipe para entrar em contato. 💜`;
          await sendViaEvolution(payload, ctx, msg);
          console.log(JSON.stringify({ evt: 'button_cancelar', clinic: payload.clinicId }));
          return jsonResponse({ ok: true, action: 'button_cancelar' });
        } catch (e) {
          errors.push(`button_cancelar: ${(e as Error).message}`);
        }
      }
      ...
      if (isNaoSouEu) {
        try {
          await executeToolByName('escalar_humano', {
            ...
            detalhes: 'Paciente clicou em "NÃO SOU EU" no lembrete de confirmação. Verificar se o número está correto.',
          }, ctx, payload, { supabaseUrl: SUPABASE_URL, serviceKey: SERVICE_ROLE_KEY });
          await sendViaEvolution(payload, ctx, 'Entendido! Vou encaminhar para nossa equipe verificar. Obrigada! 💜');
          console.log(JSON.stringify({ evt: 'button_nao_sou_eu', clinic: payload.clinicId }));
          return jsonResponse({ ok: true, action: 'button_nao_sou_eu' });
        } catch (e) {
          errors.push(`button_nao_sou_eu: ${(e as Error).message}`);
        }
      }
    }
  }
