-- ============================================================================
-- RPC: donna_load_context
-- Retorna em uma chamada todo o contexto que a Donna precisa por turno:
-- histórico, profissionais, procedimentos, clínica, paciente, lead,
-- e config Evolution (URL + master key + instance da clínica).
--
-- Uso (n8n HTTP node):
...
AS $$
  SELECT jsonb_build_object(
    -- Últimas 40 mensagens (mais antigas primeiro)
    'history', COALESCE(
      (
    ...
    -- Procedimentos ativos com preço/parcelamento/profissionais
    'procedures', COALESCE(
      (
    ...
    -- Lead mais recente (se phone bater)
    'lead', (
      SELECT jsonb_build_object(
        'id', id,
        'name', name,
    ...
    -- Config Evolution (URL + master key + instance da clínica)
...
-- ============================================================================
-- Teste rápido (substitua o phone por um da Sarah Pina)
-- ============================================================================
