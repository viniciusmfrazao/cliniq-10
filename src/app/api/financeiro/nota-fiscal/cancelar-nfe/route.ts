...
import { cancelarNfeProduto, baixarXmlAutorizado, tokenNfe } from '@/lib/focus-nfe'

export const dynamic = 'force-dynamic'

// Cancelamento de NFe (produto) — mesma regra de acesso e mesmo cuidado da NFS-e
// (ver cancelar/route.ts). A SEFAZ só aceita cancelamento em até 24h da emissão em
// geral; se estourar o prazo o erro vem da própria SEFAZ e é repassado pro usuário.
export async function POST(req: NextRequest) {
  ...
  const { data } = await cancelarNfeProduto(config, entrada.nota_fiscal_ref, justificativa.trim())
