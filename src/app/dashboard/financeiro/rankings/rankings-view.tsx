...
function RankingPacientes({ entradas }: { entradas: Entrada[] }) {
  ...
  const ranking = useMemo(() => {
    ...
    entradas.forEach(e => {
      ...
      if (e.procedimento_nome && e.tipo_receita !== 'produto') mapa[k].procs[e.procedimento_nome] = (mapa[k].procs[e.procedimento_nome] || 0) + 1
    })
...
function RankingProcedimentos({ entradas }: { entradas: Entrada[] }) {
  const data = useMemo(() => {
    ...
    entradas.filter(e => e.tipo_receita !== 'produto').forEach(e => {
...
// ─── Ranking de Produtos ────────────────────────────────────────────────────────
function RankingProdutos({ entradas }: { entradas: Entrada[] }) {
  const data = useMemo(() => {
    const mapa: Record<string, { nome: string; qtd: number; fat: number; liquido: number }> = {}
    entradas.filter(e => e.tipo_receita === 'produto').forEach(e => {
      const k = e.procedimento_nome || 'Sem nome'
      if (!mapa[k]) mapa[k] = { nome: k, qtd: 0, fat: 0, liquido: 0 }
      mapa[k].qtd += Number(e.quantidade || 1)
      mapa[k].fat += Number(e.valor_bruto || 0)
      mapa[k].liquido += Number(e.valor_liquido || 0)
    })
    return Object.values(mapa).sort((a, b) => b.fat - a.fat)
  ...
  return (
    <div className="space-y-4">
      {data.length > 0 && (
        <div className="card p-4">
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Top produtos por faturamento</p>
          <div style={{ width: '100%', height: Math.min(8, data.length) * 34 + 10 }}>
            <ResponsiveContainer>
              <BarChart data={data.slice(0, 8)} layout="vertical" margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
                <XAxis type="number" hide />
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100">
            {['#','Produto','Qtd','Faturamento','Líquido','Ticket médio','%'].map(h => (
              <th key={h} className="text-left py-3 px-3 text-xs font-semibold text-slate-400">{h}</th>
            ))}
          </tr>
        </thead>
      ...
      {data.length === 0 && <p className="text-center py-8 text-slate-400">Nenhuma venda de produto encontrada</p>}
      </div>
    </div>
  )
}
...
export default function RankingsView({ entradas }: { entradas: Entrada[] }) {
  const [aba, setAba] = useState<'pacientes' | 'procedimentos' | 'produtos'>('pacientes')
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
        ...
        <button onClick={() => setAba('produtos')}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${aba === 'produtos' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
          🛍️ Ranking de Produtos
        </button>
      </div>

      {aba === 'pacientes' && <RankingPacientes entradas={filtered} />}
      {aba === 'procedimentos' && <RankingProcedimentos entradas={filtered} />}
      {aba === 'produtos' && <RankingProdutos entradas={filtered} />}
    </div>
  )
}
