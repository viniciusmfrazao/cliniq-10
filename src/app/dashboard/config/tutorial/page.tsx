  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="DRE e relatórios" steps={['Vá em Financeiro → DRE para ver o resultado mensal','Receita bruta, deduções de taxas e lucro líquido','Rankings de procedimentos mais rentáveis e profissionais','Vendas de produto aparecem em sub-linha separada no DRE, sem misturar com procedimento','Fluxo de caixa com projeção anual','Metas mensais com progresso em tempo real']} />
        <Tutorial title="Vendas de produto no financeiro" steps={['Em Financeiro → Rankings, use a aba \"🛍️ Ranking de Produtos\" para ver o que mais vende','O Ranking de Procedimentos não mistura mais venda de produto avulso','Em Financeiro → Entradas, use os cards \"Por produto\" / \"Por procedimento\" para filtrar a lista rapidamente','Na ficha do paciente, o valor líquido (já descontada a taxa) aparece junto de cada venda de produto']} />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="Áudio nas automações" steps={['Em Configurações → Automações, cada automação (confirmação, lembrete 2h, pós-venda, aniversário) tem um modo de envio','Escolha \"Texto\", \"Áudio\" ou \"Ambos\"','No modo \"Ambos\", o texto sai primeiro e o áudio é enviado logo em seguida','Útil pra deixar a mensagem mais pessoal sem perder o registro em texto na conversa']} />
        <Tutorial title="Lembrete Personalizado" steps={['Em Configurações → Automações há uma 4ª aba: \"Lembrete Personalizado\"','Crie um lembrete com texto e horário próprios, além dos lembretes padrão de 24h e 2h','Funciona com o mesmo motor de envio (texto, áudio ou ambos)','Roda a cada 5 minutos, no horário de Brasília']} />
      </div>
      <div className="bg-violet-50 border border-violet-100 rounded-xl p-4">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="Mapa Corporal (Harmonização Corporal)" steps={['Se o módulo Harmonização Corporal estiver ativo, a ficha do paciente ganha a aba \"Mapa Corporal\"','Escolha a silhueta frente ou costas','Marque os pontos de aplicação diretamente no corpo','O sistema desenha contornos suaves entre os pontos aplicados','Fica salvo junto com o restante do histórico de procedimentos do paciente']} />
        <Tutorial title="Vender produto avulso" steps={['Na ficha do paciente, clique em \"Vender Produto\"','Adicione um ou mais produtos ao carrinho e ajuste as quantidades','Escolha a forma de pagamento — o sistema calcula taxa e valor líquido automaticamente','Confirme a venda: o estoque é descontado e a entrada financeira é criada como \"Venda de Produto\"','Também dá pra vender produto direto pelo \"Registrar Pagamento\" da agenda ou por \"Nova Entrada\" manual, marcando o tipo como Produto','Vendas acima do estoque disponível geram apenas um aviso — não bloqueiam a venda']} />
      ...
      <div className="bg-violet-50 border border-violet-100 rounded-xl p-4">
        <p className="text-sm text-violet-800"><strong>Novo:</strong> Agora dá pra vender produto avulso (sem procedimento vinculado) direto da ficha do paciente ou da agenda. Isso entra separado no financeiro, com ranking e DRE próprios.</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="Anamnese digital (modelo padrão)" steps={['O modelo padrão já vem com os campos clássicos de anamnese estética/clínica prontos','Envie o link para o paciente antes da consulta','As respostas ficam salvas na ficha do paciente','Na tela de atendimento, o profissional vê as respostas em destaque, com alertas de contraindicação quando aplicável']} />
        <Tutorial title="Anamnese personalizada (novo)" steps={['Em Configurações → Anamnese → Templates, clique em \"Novo Template\"','Monte o formulário do seu jeito: seções e perguntas próprias, estilo Google Forms','Isso é independente do modelo fixo de 37 campos — você escolhe qual template enviar para cada paciente','Os campos fixos continuam existindo e alimentando os alertas de contraindicação automáticos','Use quando precisar de um formulário específico para um procedimento ou especialidade']} />
        <Tutorial title="Anexos do paciente" steps={['Na ficha do paciente, abra a aba \"Anexos\"','Envie exames, laudos, fotos ou qualquer arquivo relevante','Fica contado com badge mostrando quantos anexos o paciente tem','Também aparece como painel resumido em outras telas da ficha']} />
        <Tutorial title="Contratos eletrônicos" steps={['Em Configurações → Contratos, crie o modelo de contrato da clínica','Envie o link de assinatura para o paciente','Ele assina pelo celular — o sistema registra IP, geolocalização e um hash de integridade do documento','O contrato assinado fica salvo automaticamente na ficha do paciente']} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        ...
        <Dica icon="🛍️" title="Venda produto sem procedimento" description="Precisa vender só um produto, sem vincular a um atendimento? Use o botão Vender Produto na ficha do paciente — não precisa criar agendamento." />
        <Dica icon="🔤" title="Busca ignora acento" description="Pode buscar paciente sem se preocupar com acentuação — 'Jose' encontra 'José' normalmente." />
      </div>
