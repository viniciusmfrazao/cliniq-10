  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Step number={8} title="Deixe a agenda com a cara da clínica (opcional)" description="Em Procedimentos, clique na bolinha ao lado de cada categoria e escolha uma cor. Os cards da agenda passam a mostrar o procedimento pela cor de fundo, e o status continua na faixa lateral." link="/dashboard/procedimentos" />
        <Step number={9} title="Comece a agendar!" description="Sua clínica está pronta. Crie o primeiro agendamento." link="/dashboard/agenda" />
      </div>
...
function TutorialAgenda() {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <Tutorial title="Criar um agendamento" steps={['Clique no botão \"+ Novo Agendamento\" no topo da tela','Selecione o paciente (ou cadastre um novo)', 'Escolha o procedimento desejado','Selecione o profissional responsável','Defina data e horário disponível','Se a clínica já negocia valor ou desconto na hora de marcar, clique em \"Aplicar valor ou desconto no agendamento\" — isso já reflete na Previsão de Faturamento','Se for sessão de pacote, veja \"Pacotes de sessões\" mais abaixo pra vincular a um pacote ou já deixar as próximas sessões marcadas','Opcionalmente registre um sinal (pagamento antecipado)','Clique em \"Confirmar Agendamento\"']} />
        ...
        <Tutorial title="Registrar pagamento (forma correta de lançar receita)" steps={['Abra o popup do agendamento e clique em \"Registrar Pagamento\"','Confira o(s) procedimento(s) — pode adicionar outro procedimento avulso na hora, se o paciente fechar mais um','Quer vender um produto junto (ex: um creme)? Clique em \"Adicionar produto\" — ele entra no MESMO carrinho, é um pagamento só','Precisa dar desconto só num item? Use o campo \"desconto R$\" ao lado dele, sem mexer no resto','Divida o pagamento em uma ou mais formas (Pix, dinheiro, crédito, débito), cada uma com sua própria taxa/parcelas calculadas automaticamente','Aplique desconto geral em valor ou percentual se for o caso — se a profissional já aplicou desconto no atendimento, ele já vem preenchido','Se o paciente tiver débito em aberto, dá pra quitar direto por aqui — inclusive parcialmente, se ele só puder pagar parte agora','Confirme — a entrada fica vinculada ao paciente e ao atendimento automaticamente, alimentando a rentabilidade do paciente']} />
        ...
        <Tutorial title="Visualizar a agenda" steps={['Use os botões \"Dia\", \"Semana\" ou \"Mês\" para trocar a visualização','Filtre por profissional usando o seletor no topo','Clique em um agendamento para ver detalhes, alterar status, editar ou abrir atendimento','A faixa colorida na lateral esquerda do card mostra o status, com um ícone dentro dela','O fundo do card mostra o procedimento — só depois que a clínica configurar as cores (veja abaixo)']} />
        <Tutorial title="Colorir a agenda por procedimento" steps={['Por padrão a agenda vem sem cores de procedimento — cada card usa a cor do status, como sempre foi','Pra ativar, vá em Procedimentos: cada categoria tem uma bolinha tracejada ao lado do nome','Clique na bolinha e escolha a cor — todos os procedimentos daquela categoria passam a usar ela na agenda','Quer uma cor só pra um procedimento específico? Abra o procedimento e use o campo \"Cor na agenda\" — ela sobrepõe a cor da categoria','Pra desfazer, clique na bolinha e escolha \"Sem cor\" — aquele grupo volta a usar a cor do status','Só admin e gerente configuram as cores; a agenda colorida todo mundo vê']} />
        <Tutorial title="Pacotes de sessões" steps={['No formulário de agendamento, se o paciente já tiver pacote ativo aparece a opção \"Vincular a um pacote ativo\"','Escolha o pacote na lista — ela já mostra quantas sessões restam','Pra um pacote novo, marque \"Criar novo pacote de sessões\" e informe nome, total de sessões e valor total','Logo abaixo aparece \"Próximas N sessões\": escolha a frequência (semanal, quinzenal, mensal) pra pré-preencher as datas, ou \"Datas manuais\" pra digitar cada uma','Cada linha continua editável — dá pra mudar data e horário de uma sessão específica sem afetar as outras','Ao salvar, todas as sessões entram na agenda de uma vez, com status \"Agendado\"','A sessão só é descontada do pacote quando aquele atendimento específico é marcado como Realizado']} />
        <Tutorial title="Como o sistema decide a cor do card" steps={['1º — cor definida no próprio agendamento, se houver','2º — cor do procedimento (campo \"Cor na agenda\" do cadastro)','3º — cor da categoria do procedimento','4º — se nada acima estiver configurado, usa a cor do status','O status NUNCA é perdido: ele sempre aparece na faixa lateral e no ícone, independente da cor do fundo','Agendamento sem procedimento definido cai direto na cor do status']} />
        <Tutorial title="Status dos agendamentos e seus ícones" steps={['📅 Agendado: consulta marcada, aguardando confirmação','🕐 Aguardando confirmação: mensagem enviada, paciente ainda não respondeu','✔️ Confirmado: paciente confirmou presença','▶️ Em atendimento: consulta em andamento','✅ Realizado: atendimento finalizado (automático ao clicar em Finalizar)','⚠️ Não compareceu: paciente faltou sem avisar','✖️ Cancelado: consulta cancelada','🔄 Reagendamento: em processo de remarcação','Cada ícone aparece dentro da faixa colorida na lateral esquerda do card. Passe o mouse por cima pra ver o nome do status','Check-in é separado do status: quando o paciente chega, o card ganha um contorno verde']} />
      ...
      <div className="bg-violet-50 border border-violet-100 rounded-xl p-4">
        <p className="text-sm text-violet-800"><strong>Importante:</strong> O jeito certo de lançar receita é pelo \"Registrar Pagamento\" no popup do agendamento — não pelo Financeiro. Isso vincula automaticamente a entrada ao atendimento e ao paciente, o que alimenta relatórios de rentabilidade.</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="Pacotes de sessões" steps={['No agendamento, marque \"Vincular a um pacote ativo\" pra descontar de um pacote já existente do paciente','Ou marque \"Criar novo pacote de sessões\": informe nome (ex: Clube do Botox), total de sessões e valor total','As próximas sessões já aparecem pré-marcadas — escolha a frequência (semanal, quinzenal ou mensal) ou selecione \"Datas manuais\" pra digitar cada uma','Cada sessão pré-marcada continua editável: dá pra ajustar data e horário individualmente antes de salvar','A sessão só é descontada do pacote quando o atendimento correspondente é marcado como Realizado — se desmarcar, a sessão volta pro pacote automaticamente','Acompanhe o progresso na tab Pacotes da ficha do paciente, em barra visual e pontos coloridos']} />
        <Tutorial title="Notas do paciente" steps={['Na ficha do paciente existe um bloco de Notas, separado do prontuário','Serve pra recado interno da equipe: preferência de horário, alergia que ela mencionou, jeito de tratar, pendência de pagamento','Fica visível pra equipe da clínica e não vai em nenhum documento nem no prontuário clínico','Diferente de Evolução (que é registro clínico do atendimento) e de Observações (que é sobre aquele agendamento específico)']} />
...
function TutorialFinanceiro() {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <Tutorial title="Registrar entrada (receita)" steps={['A forma correta é pelo popup do agendamento na Agenda, botão \"Registrar Pagamento\" — assim a entrada já vem vinculada ao paciente e ao atendimento','Procedimento e produto entram no mesmo carrinho — a paciente paga uma vez só, mesmo levando um creme junto','Selecione forma(s) de pagamento; se cartão, informe bandeira e parcelas','O sistema calcula automaticamente taxa e valor líquido','Precisa corrigir um lançamento já feito? Vá em Financeiro → Entradas, clique no lápis e edite','Venda sem atendimento vinculado (ex: venda avulsa de produto)? Financeiro → Nova Entrada abre o mesmo carrinho unificado']} />
        <Tutorial title="Emitir nota fiscal" steps={['Em Financeiro → Entradas, clique no ícone de nota fiscal ao lado do lançamento','Escolha o tipo: Serviço (NFS-e) ou Produto (NFe)','O toggle de Produto (NFe) só aparece se a clínica tiver a configuração fiscal completa em Configurações → Fiscal','Confira os dados pré-preenchidos do cadastro do paciente (CPF é opcional) antes de emitir','Qualquer pessoa com permissão de lançamento financeiro pode emitir — não precisa ser admin']} />
        ...
        <Tutorial title="Receber por boleto" steps={['No \"Registrar Pagamento\", escolha Boleto como forma de pagamento','Informe em quantas parcelas e as datas de vencimento','Cada parcela entra separada, com o vencimento próprio','Quando o paciente pagar, confirme a parcela — só aí ela vira dinheiro em caixa','Parcela vencida sem confirmação aparece em Financeiro → Devedores']} />
        <Tutorial title="Previsão de Recebimento" steps={['Vá em Financeiro → Previsão de Recebimento','Mostra o dinheiro que já foi vendido mas ainda não caiu na conta','Vendas no cartão entram na data prevista de repasse, já descontada a taxa da bandeira','Boletos entram na data de vencimento','Serve pra saber quanto realmente entra nas próximas semanas — diferente do faturamento, que conta a venda no dia em que ela aconteceu']} />
        <Tutorial title="Comissão dos profissionais" steps={['Só admin configura: Configurações → Comissões','Ligue a comissão e escolha a base de cálculo: valor bruto ou valor líquido (já descontada a taxa do cartão)','Defina o percentual de cada profissional individualmente','Profissional que não recebe comissão é só deixar desmarcado','Cada profissional acompanha o que tem a receber em \"Minhas Comissões\" — ele vê só as próprias','A comissão é calculada em cima dos atendimentos realizados no período']} />
        <Tutorial title="Quem enxerga o quê no Financeiro" steps={['Em Configurações → Permissões você controla o alcance de cada função','\"Ver todo o financeiro\": a pessoa vê os números da clínica inteira','\"Ver apenas o próprio\": a pessoa vê só os atendimentos e valores dela','As duas são excludentes — vale uma ou outra, nunca as duas','Profissional costuma ficar com \"apenas o próprio\"; recepção e financeiro com \"todo o financeiro\"','Isso vale pra DRE, rankings, previsões e comissões — a tela é a mesma, o recorte dos números é que muda']} />
        ...
        <Tutorial title="Controle de devedores" steps={['Vá em Financeiro → Devedores','Veja todos os pacientes com parcelas em aberto','Registre um pagamento clicando no débito — pode ser parcial, se o paciente só puder pagar parte agora','O sistema atualiza o saldo pro valor restante automaticamente','Parcelas vencidas ficam destacadas em vermelho']} />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Tutorial title="Permissões por função" steps={['Vá em Configurações → Permissões e escolha a função (Recepcionista, Esteticista, etc)','Marque o que aquela função pode ver e fazer em cada módulo','Dá pra ajustar uma pessoa específica sem mexer na função inteira: Equipe → o membro → Permissões','Atenção no financeiro: \"ver todo o financeiro\" e \"ver apenas o próprio\" são excludentes','A pessoa precisa sair e entrar de novo pra permissão nova valer']} />
        <Tutorial title="Auditoria — quem mexeu no quê" steps={['Vá em Auditoria pra ver o histórico de alterações da clínica','Cada registro mostra quem fez, quando, em qual módulo e o que mudou','A origem também aparece: se foi alguém pelo sistema, uma automação agendada ou acesso direto ao banco','Use quando um dado \"sumiu\" ou mudou sem explicação — quase sempre a resposta está aqui','O histórico não pode ser apagado por ninguém da clínica']} />
        <Tutorial title="Segurança da conta" steps={['Em Segurança você vê os acessos recentes à sua conta','Confira se há algum acesso que você não reconhece','Cada pessoa deve ter o próprio login — senha compartilhada quebra a auditoria, porque tudo fica no nome de uma pessoa só','Ao desligar alguém, desative o usuário em Equipe em vez de trocar a senha e repassar']} />
