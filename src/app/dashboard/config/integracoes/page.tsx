...
export default function IntegracoesPage() {
  ...
  const [evolutionStatus, setEvolutionStatus] = useState<'unconfigured' | 'testing' | 'connected' | 'error'>('unconfigured')
  ...
  async function loadEvolutionConfig() {
    ...
    if (config?.config) {
      setEvolutionConfig(config.config)
      if (config.config.url && config.config.api_key) {
  ...
  async function saveEvolutionConfig() {
    ...
    try {
      ...
      setEvolutionError(err.message || 'Erro ao salvar')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function testEvolutionConnection() {
    if (!evolutionConfig.url || !evolutionConfig.api_key) {
      setEvolutionError('Preencha URL e API Key')
      return
    }
    ...
    try {
      ...
      if (data.success) {
        setEvolutionStatus('connected')
        // Se encontrou instâncias e não tem nome configurado, sugere a primeira
        if (data.instances?.length > 0 && !evolutionConfig.instance_name) {
          setEvolutionConfig(prev => ({ ...prev, instance_name: data.instances[0].instanceName }))
        }
      } else {
        setEvolutionStatus('error')
        setEvolutionError(data.error || 'Falha na conexão')
      }
    } catch (err: any) {
      setEvolutionStatus('error')
      setEvolutionError('Erro de conexão')
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/20 dark:to-green-900/20">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">WhatsApp (Evolution API)</h2>
              <p className="text-sm text-slate-600 dark:text-slate-400">Conecte seu WhatsApp para chat integrado</p>
            </div>
            <div>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              type="url"
              className="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
          ...
          <div>
            ...
            <input
              type="password"
              className="w-full border border-slate-200 dark:border-slate-600 dark:bg-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 font-mono"
              value={evolutionConfig.api_key}
              onChange={e => setEvolutionConfig(prev => ({ ...prev, api_key: e.target.value }))}
          ...
          <div>
            ...
            <input
              ...
              onChange={e => setEvolutionConfig(prev => ({ ...prev, instance_name: e.target.value }))}
              placeholder="Ex: clinica-sarah-pina"
            />
            <p className="text-xs text-slate-500 mt-1">
              Nome da instância do WhatsApp conectado
            </p>
          </div>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50"
            >
          ...
          {evolutionStatus === 'connected' && (
            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl">
              ...
              <p className="text-xs text-emerald-600 dark:text-emerald-500">
                Acesse o menu <strong>WhatsApp</strong> para ver suas conversas.
              </p>
            </div>
          )}
        </div>
